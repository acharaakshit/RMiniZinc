...
Please make sure to install MiniZinc on your operating system and also ensure that minizinc is accessible from command line by typing 'minizinc' on the command line without the single quotes. The installation instructtiojns can be found at [instructions](https://www.minizinc.org/doc-2.4.3/en/installation.html)
...
Parameter is used to specify a parameter in a given problem. Parameters can be single types i.e integers (int), floating point numbers (float), Booleans (bool) and strings (string) but strings are only used in output formatting so they won't be used as parameter types and collections i.e sets, enums and arrays (upto 5 dimensional arrays).
