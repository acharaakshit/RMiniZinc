...
mzn_path = paste0(PROJECT_DIRECTORY, mzn_local_location, "jobshop/jobshop_0.mzn")
...
```{r, error=parse.next}
"Int$new() can be used to create a new Integer in MiniZinc"
"intExpressions() creates a sequence of Ints in MiniZinc"
"Array$new() is used to create an Array value expression"
...
# Model Items/Components

A MiniZinc model is made of several components namely, Variables (and Parameters), Constraints, Solve Items, Assignments and Annotations which are described below.
...
```{r, error=parse.next}
# IntDecl() can be used to declare an integer variable or parameter
decl = IntDecl(name = "n", kind = "par")
# VarDeclItem is the class for declaring a variable item
item1 = VarDeclItem$new(decl = decl)
# BinOp class is used to create binary arithmetic expressions
