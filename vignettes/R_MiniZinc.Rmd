...
A parser function `mzn_parse` has also been implemented which can be used to detect possible syntax errors before the MiniZinc model is evaluated. This function can either take the path of a model file path and data file path (optional) or it can take a string model along with a custom name to the string model.
...
The function `mzn_eval` can be used to evaluate a MiniZinc model. A string model should be passed to the function along with the solver name along with the path of the MiniZinc library where the solver to be used is present. A dzn file can also be provided if required. Currently, only gecode solver can be used. Once the model solutions are obtained, the solutions are parsed using the function `sol_parse()` and the results are returned as a List. If the optimal solution is found the list element containing the list can be obtained using `<solution>$optimal_solution`. More features will be added later.
