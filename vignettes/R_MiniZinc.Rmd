...
```
A parser function `mzn_parse` has also been implemented which can be used to detect possible syntax errors (and to modify the components of the model -- to be done later) before the MiniZinc model is evaluated. This function can either take the path of a model file path or it can take a string model and returns a list with two element namely: `modelString` which is the string representation of the model and `missingValues` which are the missing parameter values in a model.

The function `mzn_eval` can be used to evaluate a MiniZinc model. A string model should be passed to the function along with the solver name along with the path of the MiniZinc library where the solver to be used is present and the path of the data file (optional). Currently, only gecode solver can be used. Once the model solutions are obtained, the solutions are parsed using the function `sol_parse()` and the results are returned as a List. If the optimal solution is found the list element containing the list can be obtained using `<solution>$optimal_solution`. More features will be added later.
...
```
The data can also be provided as a list of values of parameters where the names of the list are the missing parameter names returned by `parse_mzn()`. For now, only integer, floats, bools and 1 dimensional integer arrays are supported. Alternatively, if the user wishes to provide a data file, it can be provided to the function `mzn_eval()`.
...
The examples of how to use these functions to solve optimization problems can be found in `mzn_test_examples`.
